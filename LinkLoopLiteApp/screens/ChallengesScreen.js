...
const STATUS_COLORS = {
  ...
  cancelled: { bg: '#1C1C1E', border: '#666', text: '#666', label: '❌ Cancelled' },
};
                ...
                <FadeIn delay={stagger(0, 80)}>
                  <GlassCard>
                    <View style={styles.statsRow}>
                      ...
                      <View style={styles.statItem}>
                        <Text style={[styles.statValue, { color: '#FFA500' }]}>{stats.winRate}%</Text>
...
function ChallengeCard({ challenge, accent, isMember, isPast, checking, cheering, onCheck, onCheer, onCancel }) {
  const statusStyle = STATUS_COLORS[challenge.status] || STATUS_COLORS.active;
    ...
    <View style={[styles.challengeCard, { backgroundColor: statusStyle.bg, borderLeftColor: statusStyle.border }]}>
      ...
      <View style={styles.progressSection}>
        <View style={styles.progressInfo}>
          ...
          {challenge.status === 'completed' && challenge.completedAt && (
            <Text style={[styles.daysLeft, { color: '#4A90D9' }]}>
      ...
      {!isPast && (
        <View style={styles.actionRow}>
          ...
          {isMember && onCheer && (
            <TouchableOpacity
              style={[styles.actionBtn, { borderColor: '#FFA500', backgroundColor: 'rgba(255,165,0,0.08)' }]}
              onPress={onCheer}
              disabled={cheering}
            >
              {cheering ? (
                ...
                <Text style={[styles.actionBtnText, { color: '#FFA500' }]}>🎉 Cheer!</Text>
              )}
            </TouchableOpacity>
          )}
          {!isMember && onCancel && (
...
const styles = StyleSheet.create({
  ...
  challengeDesc: { fontSize: TYPE.sm, color: '#999', marginTop: 2, lineHeight: 18 },
  ...
  progressText: { fontSize: TYPE.sm, color: '#B0B0B0' },
  daysLeft: { fontSize: TYPE.sm, color: '#FFA500', fontWeight: TYPE.bold },
  ...
  cheerName: { fontSize: TYPE.xs, color: '#888', marginTop: 1 },
  cheerMore: { fontSize: TYPE.sm, color: '#666' },

  // Actions
  actionRow: { flexDirection: 'row', gap: 10 },
  actionBtn: { flex: 1, borderRadius: 10, paddingVertical: 10, alignItems: 'center', borderWidth: 1, backgroundColor: 'rgba(255,255,255,0.04)' },
  ...
  modalSubtitle: { fontSize: TYPE.md, color: '#888', marginBottom: 16 },
  templateList: { maxHeight: 500 },
  templateCard: {
    flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.06)',
    borderRadius: 14, padding: 16, marginBottom: 12, borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)',
  },
  ...
  templateDesc: { fontSize: TYPE.sm, color: '#999', lineHeight: 18, marginBottom: 4 },
  templateMeta: { fontSize: TYPE.xs, color: '#666' },
});
